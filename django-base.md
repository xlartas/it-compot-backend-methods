...
### Http методы, form
   ...
   а в ответ мы получаем `html` страницу (ответ может быть не только в 
   формате `html`, может `json`, может `xml` или просто текст...).<br>
   ...
   `action`: Ссылка, куда будут отправляться данные. Если не указывать, то на ту же страницу.
   ...
   * ### GET
     ...
     * http://127.0.0.1:8000/?param1=value_1&param2=abcd&param3=321&param4=sometext/
     > Перед передачей параметров ставиться `?`, а после пары ключ-значение разделяются амперсандами `&`.
   * ### POST
   ...
   * ### PATCH
     ...
     изменений в уже существующий ресурс(запись в бд...).<br>
   * ### PUT
...
* `ManyToManyField(to=Product)` Поле для создания связи "многие ко многим" с другой моделью Product.
...
* #### Related Name
    ...
    Когда вы создаете отношение в модели `Django`, например, используя `ForeignKey`, `Django` 
...
извне(админка, пользовательский интерфейс)<br>
Они точно также обслуживаются сервером и доступны по ссылке, как и статические файлы, но хранятся немного иначе.
```python
# models.py
