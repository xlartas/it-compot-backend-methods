...
- [Шаблонизация](#шаблонизация)
  ...
  - [Расширение/Наследование шаблонов](#Расширение/Наследование-шаблонов)
...
1. Добавляем в маршрут динамическую часть `<int:id>`.
   ```python
      # urls.py
      urlpatterns = [
          ...
...
2. В файле views.py создаем функцию `example_view` которая будет возвращать отрендеренный шаблон `products.html`.
   ```python
      ...
      def example_view(request, id, text):  
          # Дополнительные параметры после request должны 
...
### URL naming & redirects (Именование маршрутов и перенаправление)
   ...
   * #### Примеры
     ...
     urlpatterns = [
         ...
         path('shop/products/', example_view, name='example'),  # задаём имя для паттерна
     ]
     ```
     * Шаблонизация ссылок:
          ```html
          <a href="{% url 'example' %}">Кликни на меня</a>
     ...
     * Перенаправление:
          ...
          def some_view(request):
              ...
              # То есть мы перенаправляем пользователя на другой адрес.
          ```
...
Добавляем третий параметр `name` в функцию маршрута.
```python
    # urls.py
    urlpatterns = [
        path('shop/products/<int:id>/<str:text>/', example_view, name='example_name'),
    ]
```
Используем это имя в шаблоне.
```html
<!-- example.html -->
<body>
    <a href="{% url 'example_name' %}"></a>
</body>
```
...
📦PROJECT_NAME `Корневая папка проекта`<br>
 ...
 ┃    ┃  ┣---📂app1 `Шаблоны app1 приложения`<br>
 ┃    ┃  ┃  ┗---📜example.html<br>
